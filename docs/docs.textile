...
Com a tag <tt><geometry></tt> determinamos o posicionamento do elemento - <tt>x</tt>, <tt>y</tt> - a largura e a altura - <tt>width</tt>, <tt>height</tt>.
...
A propriedade color, que determina a cor da linha do sublinhado, pode ser omitida.
...
A propriedade thickness, que determina a espessura da borda, pode ser omitida.

h3. Ações sobre os elementos

Podemos especificar uma ação a ser executada quando um elemento for clicado.
Essa ação pode ser abrir uma página, fazer o play ou o pause do vídeo, ou até abrir outro vídeo.

Para configurar a ação de abrir uma página, fazemos:
...
A tag <tt><link></tt> é a que utilizamos para configurar a ação. Essa tag aceita opcionalmente as configurações de tooltip e de thumbnail.
...
É possível definir a cor da borda através do atributo color, na tag frame.

É possível também definir a espessura da borda através do atributo thickness. Essa espessura é medida em pixels.

<pre>
<frame color="0x0000FF" thickness="2"/>
</pre>
